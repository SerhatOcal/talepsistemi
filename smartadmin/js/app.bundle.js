	...
	app.listFilter = function (list, input, anchor) {
		...
		$(input).change( function () {
			...
			if(filter.length > 1) {
				...
				if (listPrev){	
					listPrev.text("toplam " + $(list).find('[data-filter-tags]').length + " kayıttan " + $(list).find('li.js-filter-show').length + " eşleşti");
				}
				...
				switch ( true ) {
					...
					case ( actiontype === 'panel-close' ):
						...
						if (typeof bootbox  != 'undefined') {
							...
							bootbox.confirm({
								title: "<i class='fal fa-times-circle text-danger mr-2'></i> Paneli ekrandan kaldırmak istiyormusunuz <span class='fw-500'>&nbsp;'" +selectedPanel.children('.panel-hdr').children('h2').text().trim()+ "'&nbsp;</span>?",
								message: "...",
								centerVertical: true,
								swapButtonOrder: true,
								buttons: {
					...
					case ( actiontype === 'lang' ):
						...
						//jQuery.getScript('http://url/to/the/script');
							...
							initApp.loadScript("/smartadmin/js/i18n/i18n.js", 
